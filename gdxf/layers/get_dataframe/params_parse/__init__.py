...
def params_parse(apis_copy, apis):
    """
    将获得的参数以表名为键的字典形式返回
    ...
    waiting_for_search = []
    table1 = {}
    # parsed_content = kwargs.get("parsed_content")  # 从参数检查中获得的内容

    # 1.1 解析第一张表的表名[table]
    # real_table = kwargs.get("real_table")
    table1["table"] = apis_copy.get("table")
    table1["ex_table"] = apis_copy.get("ex_table")

    # 1.2 解析第一张表的字段[columns]
    columns = f'{apis_copy.get("name", "")},{apis_copy.get("stack", "")},{apis_copy.get("value", "")}'
    columns = [i for i in columns.split(",") if i]  # 从参数中得到第一张表的字段
    table1["columns"] = columns

    # 1.3 解析第一张表的条件[conditions]
    table1["conditions"] = []
    code, msg, condition1_1 = get_conditions(apis_copy.get("ex_table"), apis)  # 把剩下的条件传过去得到条件
    if code != 200:
        return code, msg, {}
    table1["conditions"].append(condition1_1)

    # 1.3 解析第一张表的transformer条件[conditions]
    transformer = apis_copy.get("transformer", "")  # 重点解析对象
    ...
    {"realm": "xf", "index": "xfjc", "name": "djjg", "transformer": "@groupby:djjg+@tb", "day":"[2020-02-02,2020-02-03]"}
    """
    code, msg, condition1_2 = parse_transformer_conditions(transformer, apis_copy.get("ex_table"), apis)
    if code != 200:
        return code, msg, {}
    if condition1_2:
