...
from layers.get_dataframe.merge_serached_dataframes.convert_and_compute.parse_groupby import parse_groupby
from layers.get_dataframe.merge_serached_dataframes.convert_and_compute.parse_zb import parse_zb
...
def merge_serached_dataframes(df_list, apis_copy):
    ...
    # 2. 转成df
    # df_list = simple2df(results, apis_copy)  # simple2df 简单转成df，不会替换 name 和 stack，得到df_list，后续的tb/hb 需要用两个df
