...
def get_sql_map(flag):
    ...
    sql_map = {
        # 区划分布 --> 全部 --> 总体 【一个数】 省级
        "aqdflv_qh_all_shej": (
            f"select sum(a.aqdf)/(sum(a.aqdf)+sum(a.cqdf)+sum(a.wdf)+sum(a.cqwdf)) as aqdflv from {base_sql_map['3-2-全']} as a",
            ["aqdflv"]),
        # 区划分布 --> 全部 --> 总体 【一个数】 市级
        "aqdflv_qh_all_shij": (
        ...
        # 区划分布 --> 信访形式 --> 总体 【一个数】 省级
        "aqdflv_qh_xfxs_shej": (
            f"select sum(a.aqdf)/(sum(a.aqdf)+sum(a.cqdf)+sum(a.wdf)+sum(a.cqwdf)) as aqdflv from {base_sql_map['3-2-形式']} as a",
            ["aqdflv"]),
        # 区划分布 --> 信访形式 --> 总体 【一个数】 市级
        "aqdflv_qh_xfxs_shij": (
        ...
        "aqdflv_qh_all_allshej": (
            ...
            ["qh", "aqdflv"]),
        # 区划分布 --> 全部 --> 总体 【name/value】 市级 排名其实就是县级
