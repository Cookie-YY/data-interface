...
def get_sql_map(flag):
    ...
    sql_map = {
        ...
        "jssllv_qh_all_xfbm_shej": (
            f"select sum(a.jssl)/sum(a.zs) as jssllv from {base_sql_map['1-2-全']} as a",
            ["jssllv"]),
        # 区划分布 --> 全部 --> 信访部门 （省） 【三种情况】 *
        "jssllv_qh_all_xfbm_shej_nums": (
            f"SELECT case a.jssl when a.jssl then '及时受理' end as slqk,sum(a.jssl) as 'xfjc' FROM {base_sql_map['1-2-全']} as a UNION SELECT case a.cqsl when a.cqsl then '超期受理' end as slqk ,sum(a.cqsl) as 'xfjc' FROM {base_sql_map['1-2-全']} as a UNION SELECT case a.cqwsl when a.cqwsl then '超期未受理' end as slqk ,sum(a.cqwsl) as 'xfjc' FROM {base_sql_map['1-2-全']} as a",
            ["slqk", "xfjc"]),
        ...
        "jssllv_qh_all_xfbm_allshej": (
            f"select a.region_name as qh,a.jssl/a.zs as jssllv from {base_sql_map['1-2-全']} as a",
            ["qh", "jssllv"]),
        ...
        "jssllv_qh_all_xfbm_xj":  (
            ...
            ["jssllv"]),
        # 区划分布 --> 全部 --> 信访部门 （某个市的某个县） 【三种情况】 *
        "jssllv_qh_all_xfbm_xj_nums": (
        ...
        "jssllv_qh_all_zrdw_shej": (
            f"select sum(a.jssl)/sum(a.zs) as jssllv from {base_sql_map['2-2-全']} as a",
            ["jssllv"]),
        # 区划分布 --> 全部 --> 责任单位 （省） 【三种情况】 *
        "jssllv_qh_all_zrdw_shej_nums": (
            f"SELECT case a.jssl when a.jssl then '及时受理' end as slqk,sum(a.jssl) as 'xfjc' FROM {base_sql_map['2-2-全']} as a UNION SELECT case a.cqsl when a.cqsl then '超期受理' end as slqk ,sum(a.cqsl) as 'xfjc' FROM {base_sql_map['2-2-全']} as a UNION SELECT case a.cqwsl when a.cqwsl then '超期未受理' end as slqk ,sum(a.cqwsl) as 'xfjc' FROM {base_sql_map['2-2-全']} as a",
            ["slqk", "xfjc"]),
        ...
        "jssllv_qh_all_zrdw_allshej": (
            f"select a.region_name as qh,a.jssl/a.zs as jssllv from {base_sql_map['2-2-全']} as a",
            ["qh", "jssllv"]),
        # 区划分布 --> 全部 --> 责任单位 （某个市的某个县） 【value】
        "jssllv_qh_all_zrdw_xj": (
        ...
        "jssllv_qh_all_zrdw_allshij": (
            ...
            ["qh", "jssllv"]),
        # 区划分布 --> 全部 --> 责任单位 （某县） 【三种情况】 *
        "jssllv_qh_all_zrdw_xj_nums": (
        ...
        "jssllv_qh_xfxs_xfbm_shej": (
            f"select sum(a.jssl)/sum(a.zs) as jssllv from {base_sql_map['1-2-形式']} as a where a.xfxs='{{xfxs}}'",
            ["jssllv"]),
        # 区划分布 --> 信访形式 --> 信访部门 （省） 【三种情况】 *
        "jssllv_qh_xfxs_xfbm_shej_nums": (
            f"SELECT case a.jssl when a.jssl then '及时受理' end as slqk,sum(a.jssl) as 'xfjc' FROM {base_sql_map['1-2-形式']} as a UNION SELECT case a.cqsl when a.cqsl then '超期受理' end as slqk ,sum(a.cqsl) as 'xfjc' FROM {base_sql_map['1-2-形式']} as a UNION SELECT case a.cqwsl when a.cqwsl then '超期未受理' end as slqk ,sum(a.cqwsl) as 'xfjc' FROM {base_sql_map['1-2-形式']} as a",
            ["slqk", "xfjc"]),
        # 区划分布 --> 信访形式 --> 信访部门 （xx市） 【一个数】
        "jssllv_qh_xfxs_xfbm_shij": (
            f"select a.jssl/a.zs as jssllv from {base_sql_map['1-2-形式']} as a where a.xfxs='{{xfxs}}'",
            ["jssllv"]),
        # 区划分布 --> 信访形式 --> 信访部门 （xx市） 【三种情况】 *
        "jssllv_qh_xfxs_xfbm_shij_nums": (
            f"SELECT case a.jssl when a.jssl then '及时受理' end as slqk,a.jssl as 'xfjc' FROM {base_sql_map['1-2-形式']} as a WHERE a.region_name='{{Cqh}}' UNION SELECT case a.cqsl when a.cqsl then '超期受理' end as slqk ,a.cqsl as 'xfjc'  FROM {base_sql_map['1-2-形式']} as a WHERE a.region_name='{{Cqh}}' UNION SELECT case a.cqwsl when a.cqwsl then '超期未受理' end as slqk ,a.cqwsl as 'xfjc' FROM {base_sql_map['1-2-形式']} as a where a.region_name='{{Cqh}}'",
            ["slqk", "xfjc"]),
        # 区划分布 --> 信访形式 --> 信访部门 （各个市） 【name/value】
        "jssllv_qh_xfxs_xfbm_allshej": (
            f"select a.region_name as qh,a.jssl/a.zs as jssllv from {base_sql_map['1-2-形式']} as a where a.xfxs='{{xfxs}}'",
            ["qh", "jssllv"]),
        # 区划分布 --> 信访形式 --> 信访部门 （某个市的某个县） 【value】
        "jssllv_qh_xfxs_xfbm_xj": (
            f"select a.jssl/a.zs as jssllv from {base_sql_map['1-4-形式']} as a where a.xfxs='{{xfxs}}'",
            ["jssllv"]),
        # 区划分布 --> 信访形式 --> 信访部门 （某个市的某个县） 【三种情况】 *
        "jssllv_qh_xfxs_xfbm_xj_nums": (
            f"SELECT case a.jssl when a.jssl then '及时受理' end as slqk,a.jssl as 'xfjc' FROM {base_sql_map['1-4-形式']} as a WHERE a.region_name='{{Cqh}}' UNION SELECT case a.cqsl when a.cqsl then '超期受理' end as slqk ,a.cqsl as 'xfjc'  FROM {base_sql_map['1-4-形式']} as a WHERE a.region_name='{{Cqh}}' UNION SELECT case a.cqwsl when a.cqwsl then '超期未受理' end as slqk ,a.cqwsl as 'xfjc' FROM {base_sql_map['1-4-形式']} as a where a.region_name='{{Cqh}}'",
            ["slqk", "xfjc"]),
        ...
        "jssllv_qh_xfxs_zrdw_shej": (
            f"select sum(a.jssl)/sum(a.zs) as jssllv from {base_sql_map['2-2-形式']} as a where a.xfxs='{{xfxs}}'",
            ["jssllv"]),
        # 区划分布 --> 信访形式 --> 责任单位 （省） 【三种情况】 *
        "jssllv_qh_xfxs_zrdw_shej_nums": (
            f"SELECT case a.jssl when a.jssl then '及时受理' end as slqk,sum(a.jssl) as 'xfjc' FROM {base_sql_map['2-2-形式']} as a UNION SELECT case a.cqsl when a.cqsl then '超期受理' end as slqk ,sum(a.cqsl) as 'xfjc' FROM {base_sql_map['2-2-形式']} as a UNION SELECT case a.cqwsl when a.cqwsl then '超期未受理' end as slqk ,sum(a.cqwsl) as 'xfjc' FROM {base_sql_map['2-2-形式']} as a",
            ["slqk", "xfjc"]),
        # 区划分布 --> 信访形式 --> 责任单位 （xx市） 【一个数】
        "jssllv_qh_xfxs_zrdw_shij": (
            f"select a.jssl/a.zs as jssllv from {base_sql_map['2-2-形式']} as a where a.xfxs='{{xfxs}}'",
            ["jssllv"]),
        # 区划分布 --> 信访形式 --> 责任单位 （市） 【三种情况】
        "jssllv_qh_xfxs_zrdw_shij_nums": (
            f"SELECT case a.jssl when a.jssl then '及时受理' end as slqk,a.jssl as 'xfjc' FROM {base_sql_map['2-2-形式']} as a WHERE a.region_name='{{Cqh}}' UNION SELECT case a.cqsl when a.cqsl then '超期受理' end as slqk ,a.cqsl as 'xfjc'  FROM {base_sql_map['2-2-形式']} as a WHERE a.region_name='{{Cqh}}' UNION SELECT case a.cqwsl when a.cqwsl then '超期未受理' end as slqk ,a.cqwsl as 'xfjc' FROM {base_sql_map['2-2-形式']} as a where a.region_name='{{Cqh}}'",
            ["slqk", "xfjc"]),
        # 区划分布 --> 信访形式 --> 责任单位 （各个市） 【name/value】
        "jssllv_qh_xfxs_zrdw_allshej": (
            f"select a.region_name as qh,a.jssl/a.zs as jssllv from {base_sql_map['2-2-形式']} as a where a.xfxs='{{xfxs}}'",
            ["qh", "jssllv"]),
        # 区划分布 --> 信访形式 --> 责任单位 （某个市的某个县） 【value】
        "jssllv_qh_xfxs_zrdw_xj": (
            f"select a.jssl/a.zs as jssllv from {base_sql_map['2-4-形式']} as a where a.xfxs='{{xfxs}}'",
            ["jssllv"]),
        # 区划分布 --> 信访形式 --> 责任单位 （某县） 【三种情况】 *
        "jssllv_qh_xfxs_zrdw_xj_nums": (
            f"SELECT case a.jssl when a.jssl then '及时受理' end as slqk,a.jssl as 'xfjc' FROM {base_sql_map['2-4-形式']} as a WHERE a.region_name='{{Cqh}}' UNION SELECT case a.cqsl when a.cqsl then '超期受理' end as slqk ,a.cqsl as 'xfjc'  FROM {base_sql_map['2-4-形式']} as a WHERE a.region_name='{{Cqh}}' UNION SELECT case a.cqwsl when a.cqwsl then '超期未受理' end as slqk ,a.cqwsl as 'xfjc' FROM {base_sql_map['2-4-形式']} as a where a.region_name='{{Cqh}}'",
            ["slqk", "xfjc"]),
...
class Jssllv(Extension):
    ...
    def before_search(self):
        ...
        if sql_item in ["jssllv_zrdw_all_allshej", "jssllv_zrdw_xfxs_allshej"]:
